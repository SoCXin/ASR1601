...
'''
PROJECT_NAME = "QuecPython_xiaobaipai_electronic scale"
...
# productKey = "a1EgVkqKt1O"  # 产品标识(参照阿里云应用开发指导)
# productSecret = None  # 产品密钥（使用一机一密认证时此参数传入None，参照阿里云应用开发指导)
# DeviceName = "xiaobaipai_balance"  # 设备名称(参照阿里云应用开发指导)
# DeviceSecret = "26d5484ed9904dfdb224ee77db7edbd7"  # 设备密钥（使用一型一密认证此参数传入None，免预注册暂不支持，需先在云端创建设备，参照阿里云应用开发指导)

productKey = "a1kYaC75CM5"  # 产品标识(参照阿里云应用开发指导)
productSecret = None  # 产品密钥（使用一机一密认证时此参数传入None，参照阿里云应用开发指导)
DeviceName = "iuI1M4EuGFWQh8aPRNM9"  # 设备名称(参照阿里云应用开发指导)
DeviceSecret = "cd6a1245fc16bb38ea0480d017fcf8f3"  # 设备密钥（使用一型一密认证此参数传入None，免预注册暂不支持，需先在云端创建设备，参照阿里云应用开发指导)


state = 5

w={'id': '123','version': '1.0','params': {'Weight': 12.6},'method': 'thing.event.property.post'} #发送数据格式
...
if __name__ == '__main__':
    ...
    # 设置mqtt连接属性
    clientID = "client_id_test"  # 自定义字符（不超过64）
    ali.setMqtt(clientID, clean_session=False, keepAlive=300)

    # 设置回调函数
    ali.setCallback(sub_cb)
    #topic = "/sys/a1EgVkqKt1O/xiaobaipai_balance/thing/event/property/post"  # 云端自定义或自拥有的Topic
    topic = "/sys/a1kYaC75CM5/iuI1M4EuGFWQh8aPRNM9/thing/event/property/post"  # 云端自定义或自拥有的Topic

    # 订阅主题
    # ali.subscribe(topic)
    # 发布消息
    w['params']['Weight'] = 19.8
    ww = ujson.dumps(w)
    ali.publish(topic, ww)
    # 运行
    print('aliyun start1')
    ali.start()
    print('aliyun start2')
    count1 = 0
    zero_ad_val = hx711_read(hx711_sck_gpio12, hx711_dout_gpio11)
#微信公众号关注 白话物联网 获取持续更新  淘宝iotnow.taobao.com
    while 1:
        weight = 0
        sum_weight = 0
        ...
        if(char_len > DIS_MAX_CHAR_NUM):
            ...
            lcd_show_ascii_str(DIS_X_START, DIS_Y_START, 16, 32, '    ', fc, bc)
            lcd_show_ascii_str(DIS_X_START + 16 * 4, DIS_Y_START, 16, 32, str(weight), fc, bc)
